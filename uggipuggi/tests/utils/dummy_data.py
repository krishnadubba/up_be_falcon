food_gcs_base = 'https://storage.googleapis.com/up_food_pics/'
users_gcs_base = 'https://storage.googleapis.com/up_users_avatars/'

users = [    
        {
            'id': 'U0001',
            'name': 'Abbie Pratt',
            'avatar': 'assets/img/jessica.png',
            'sex': 'female',
            'rank': 'popular',
            'repicies': 7120
            },
        {
            'id': 'U0002',
            'avatar': 'assets/img/patrick.png',
            'name': 'Patrick Garza',
            'sex': 'male',
            'rank': 'popular',
            'repicies': 2120
            },
        {
            'id': 'U0003',
            'avatar': 'assets/img/louis.png',
            'name': 'Louis Ortiz',
            'sex': 'male',
            'rank': 'popular',
            'repicies': 7320
            },
        {
            'id': 'U0004',
            'avatar': 'assets/img/eula.png',
            'name': 'Eula King',
            'sex': 'female',
            'rank': 'popular',
            'repicies': 5120
            },
        {
            'id': 'U0005',
            'avatar': 'assets/img/duane.png',
            'name': 'Duane Lane',
            'sex': 'female',
            'rank': 'popular',
            'repicies': 6120,
            'public_profile': True
            },
        {
            'id': 'U0006',
            'avatar': 'assets/img/katharine.png',
            'name': 'Kathazine',
            'sex': 'female',
            'rank': 'popular',
            'repicies': 7220
            },
        {
            'id': 'U0007',
            'avatar': 'assets/img/profile-anhlq.png',
            'name': 'Quang Anh',
            'sex': 'male',
            'rank': 'popular',
            'repicies': 7120
            },
        {
            'id': 'U0008',
            'avatar': 'assets/img/banana.png',
            'name': 'Banana Man',
            'sex': 'male',
            'rank': 'recent',
            'repicies': 7120
            },
        {
            'id': 'U0009',
            'avatar': 'assets/img/chris.png',
            'name': 'Chris',
            'sex': 'male',
            'rank': 'recent',
            'repicies': 7120
            },
        {
            'id': 'U0010',
            'avatar': 'assets/img/demi.png',
            'name': 'Demi',
            'sex': 'female',
            'rank': 'other',
            'repicies': 7120,
            'public_profile': True
            },
        {
            'id': 'U0011',
            'avatar': 'assets/img/franse.png',
            'name': 'Franse',
            'sex': 'male',
            'rank': 'other',
            'repicies': 7120
            },
        {
            'id': 'U0012',
            'avatar': 'assets/img/kazi.png',
            'name': 'Kazi',
            'sex': 'male',
            'rank': 'other',
            'repicies': 7120
            },
        {
            'id': 'U0013',
            'avatar': 'assets/img/ollie.png',
            'name': 'Oliie',
            'sex': 'male',
            'rank': 'other',
            'repicies': 7120
            },
        {
            'id': 'U0014',
            'avatar': 'assets/img/salam.png',
            'name': 'Salam',
            'sex': 'male',
            'rank': 'recent',
            'repicies': 7120
            },
        {
            'id': 'U0015',
            'avatar': 'assets/img/utre.png',
            'name': 'Le Minh',
            'sex': 'female',
            'rank': 'recent',
            'repicies': 7120
            },
        {
            'id': 'U0016',
            'name': 'Jeniffer Cruise',
            'avatar': 'assets/img/jeniffer.png',
            'sex': 'female',
            'rank': 'popular',
            'repicies': 7120
            },        
        ];

groups = [{'group_name':'Alpha', 'group_pic':'', 'admin':'U0001', 'members':['U0002','U0006','U0007','U0008','U0010']},
          {'group_name':'Beta',  'group_pic':'', 'admin':'U0002', 'members':['U0011','U0009','U0008']},
          {'group_name':'Theta', 'group_pic':'', 'admin':'U0003', 'members':['U0015','U0012','U0008']},
          {'group_name':'Gamma', 'group_pic':'', 'admin':'U0004', 'members':['U0005','U0010','U0008']},]

contacts = [('U0009','U0001','U0002','U0003','U0004','U0005','U0006','U0007',
             'U0008','U0010','U0011','U0012','U0013','U0014','U0015'),
            ('U0007','U0001','U0002','U0003','U0004','U0005','U0006','U0008',
             'U0009','U0010','U0011','U0012','U0013','U0014','U0015'),
            ('U0011','U0001','U0002','U0003','U0004','U0005','U0006','U0007',
             'U0008','U0009','U0010','U0012','U0013','U0014','U0015'),
            ('U0006','U0001','U0002','U0003','U0004','U0005','U0007','U0008',
             'U0009','U0010','U0011','U0012','U0013','U0014','U0015')]

#contacts = [('U0009','U0002','U0006','U0007','U0012','U0010'),
            #('U0007','U0014','U0011','U0016','U0009','U0012',),
            #('U0011','U0015','U0016','U0012'),
            #('U0006','U0002','U0001','U0009','U0016','U0010')]

following = [('U0001','U0014','U0013','U0015','U0008','U0010'),
            ('U0002','U0004','U0001','U0013','U0009','U0011',),
            ('U0003','U0005','U0006','U0012'),
            ('U0004','U0002','U0001','U0005','U0013','U0010')]

materials = [
    {
        'id': 'M0001',
        'name': 'Fresh Lemon',
        'image': 'assets/img/lemon.png',
        },
    {
        'id': 'M0002',
        'name': 'Ice Cube',
        'image': 'assets/img/icecube.png',
        },
    {
        'id': 'M0003',
        'name': 'Water',
        'image': 'assets/img/water.png',
        },
    {
        'id': 'M0004',
        'name': 'Salt',
        'image': 'assets/img/salt.png',
        },
    {
        'id': 'M0005',
        'name': 'CrawFish Tails',
        'image': 'assets/img/crawfish-tails.png',
        },
    {
        'id': 'M0006',
        'name': 'Mushroom',
        'image': 'assets/img/mushrooms.png',
        },
    {
        'id': 'M0007',
        'name': 'Roma Tomato',
        'image': 'assets/img/roma-tomato.png',
        },
    {
        'id': 'M0008',
        'name': 'Red Bell Pepper',
        'image': 'assets/img/red-bell-pepper.png',
        },
    {
        'id': 'M0009',
        'name': 'Pasta',
        'image': 'assets/img/pasta.png',
        },
    {
        'id': 'M0010',
        'name': 'Tasso',
        'image': 'assets/img/tasso.png',
        },
    {
        'id': 'M0011',
        'name': 'Parmesan Cheese',
        'image': 'assets/img/parmesan-cheese.png',
        },
    {
        'id': 'M0012',
        'name': 'Romaine Head',
        'image': 'assets/img/romaine_head.png',
        },
    {
        'id': 'M0013',
        'name': 'Gala Apple',
        'image': 'assets/img/gala_apple.png',
        },
    {
        'id': 'M0014',
        'name': 'White Balsamic Vinaigrette',
        'image': 'assets/img/white_balsamic_vinaigrette.png',
        },
    {
        'id': 'M0015',
        'name': 'Bacon',
        'image': 'assets/img/bacon.png',
        },
    {
        'id': 'M0016',
        'name': 'Blue Cheese',
        'image': 'assets/img/blue_cheese.png',
        },
    {
        'id': 'M0017',
        'name': 'Green Onion',
        'image': 'assets/img/green_onion.png',
        },
  
    {
        'id': 'M0018',
        'name': 'Roasted Eggplant',
        'image': 'assets/img/eggplant.png',
        },
    {
        'id': 'M0019',
        'name': 'Roasted Red Pepper',
        'image': 'assets/img/roasted_red_pepper.png',
        },
    {
        'id': 'M0020',
        'name': 'Artichokes',
        'image': 'assets/img/artichokes.png',
        },
    {
        'id': 'M0021',
        'name': 'Greens and Onions',
        'image': 'assets/img/greens_and_onions.png',
        },
    {
        'id': 'M0022',
        'name': 'Olives and Roasted Garlic',
        'image': 'assets/img/olives_and_roasted_garlic.png',
        },
    {
        'id': 'M0023',
        'name': 'Mozzarella',
        'image': 'assets/img/mozzarella.png',
        },
    {
        'id': 'M0024',
        'name': 'Kimchi and Chilli Garlic Sause',
        'image': 'assets/img/kimchi_and_chilli_garlic_sause.png',
        },
    {
        'id': 'M0025',
        'name': 'Scallious',
        'image': 'assets/img/scallious.png',
        },
    {
        'id': 'M0026',
        'name': 'Cooked Noodles',
        'image': 'assets/img/cooked_noodles.png',
        },
    {
        'id': 'M0027',
        'name': 'Beef Base',
        'image': 'assets/img/beef_base.png',
        },
    {
        'id': 'M0028',
        'name': 'Shiitake Mushrooms',
        'image': 'assets/img/shiitake_mushrooms.png',
        },
    {
        'id': 'M0029',
        'name': 'Beef Jerky',
        'image': 'assets/img/beef_jerky.png',
        },    
    {
        'id': 'M0030',
        'name': 'Garlic Bread',
        'image': 'assets/img/garlic_bread.png',
        },
    {
        'id': 'M0031',
        'name': 'Hearty Tomato Sauce',
        'image': 'assets/img/hearty_tomato_sauce.png',
        },
    {
        'id': 'M0032',
        'name': 'Whole Milk Mozzarella',
        'image': 'assets/img/whole_milk_mozzarella.png',
        },
    {
        'id': 'M0033',
        'name': 'Fresh Parsley',
        'image': 'assets/img/fresh_parsley.png',
        },        
    {
        'id': 'M0034',
        'name': 'Frangelico',
        'image': 'assets/img/frangelico.png',
        },
    {
        'id': 'M0035',
        'name': 'Spiced Rum',
        'image': 'assets/img/spiced_rum.png',
        },
    {
        'id': 'M0036',
        'name': 'Coca-Cola',
        'image': 'assets/img/coke.png',
        },
    {
        'id': 'M0037',
        'name': 'Ice',
        'image': 'assets/img/rock_glass.png',
        },                     
   ]


recipes = [
    {
      'id': 'R0001',
      'name': 'City Park Salad',
      'image': 'assets/img/city_spark_salad.png',
      'author': users[4],
      'direction': """<b>White Balsamic Vinaigrette:</b>
      <ul>
        <li>Add all ingredients or a mixing bowl or squeeze bottle.</li>
        <li>Whisk or shake well to combine ingredients.</li>
        <li>Set aside until ready to dress the salads.</li>
      </ul>
      <b>Salad Assembly:</b>
      <ul>
        <li>Cook bacon over medium high heat in a skillet until crispy. Set aside to cool and drain on paper towels. Chop once cool.</li>
        <li>Place romaine on two plates and top each with half of the dressing.</li>
        <li>Finish the salads by topping each with half of the bacon, apple, blue cheese and green onion.</li>
        <li>Serve immediately.</li>
      </ul>""",
      'ingredients': [
        {
          'material': materials[11],
          'unit': 'Heads',
          'quantity': '1 large or 2 small'
        },
        {
          'material': materials[12],
          'unit': 'Cup',
          'quantity': '1/2'
        },
        {
          'material': materials[13],
          'unit': 'Cup',
          'quantity': '1/4'
        },
        {
          'material': materials[14],
          'unit': 'Slices',
          'quantity': '3'
        },
        {
          'material': materials[15],
          'unit': 'Tablespoons Crumbled',
          'quantity': '4'
        },
        {
          'material': materials[16],
          'unit': 'Tablespoons Crumbled',
          'quantity': '4'
        },
      ],
      'likes': 0,
      'reviews': [
        {
          'author': users[2],
          'content': 'Ok I am officially starving – this looks fantastic! Love the salty of bacon with the sweet apple. NICE!!',
          'created_date': ""
        },
        {
          'author': users[8],
          'content': 'Thanks, Liz I love this recipe. Apples, blue cheese and bacon are one of my favorite flavor combos too',
          'created_date': ""
        },
        {
          'author': users[5],
          'content': 'Very nice salad…',
          'created_date': ""
        },
      ]
    },
    
    {
        'id': 'R0002',
        'name': 'Roasted Veggie Flatbread Pizza',
        'image': 'assets/img/roasted-veggie-flatbread-pizza.png',
        'author': users[6],
        'direction':"""<ul>
        <li>Preheat the oven to 400 ºF.</li>
        <li>In a medium large skillet over medium high heat, combine the onion, salt, pepper and neutral oil. Cook for 10-15 minutes until onion is caramelized.</li>
        <li>Add the hearty salad greens and toss well to combine. Remove from heat as soon as salad greens are softened.</li>
        <li>Place the four naan flatbreads between two baking sheets. Divide the prepared topping ingredients into four portions, and top each bread with a quarter of the ingredients. Save the mozzarella and green onions for last - use those to top all of the other ingredients.</li>
        <li>Bake for 10 minutes. Remove from oven and allow to cool 5 minutes before cutting.</li>
        </ul>"""
        ,
        'ingredients': [
            {
                'material': materials[17],
                'unit': 'Cup Cubed',
                'quantity': '1'
                },
            {
                'material': materials[18],
                'unit': 'Tablespoons Chopped',
                'quantity': '4'
                },
            {
                'material': materials[19],
                'unit': 'Tablespoons Chopped',
                'quantity': '4'
                },
            {
                'material': materials[20],
                'unit': 'Tablespoons Thinly Sliced',
                'quantity': '2'
                },
            {
                'material': materials[21],
                'unit': 'Tablespoons Chopped',
                'quantity': '4'
                },
            {
                'material': materials[22],
                'unit': 'Cup Grated',
                'quantity': '1'
                },
            ],
        'likes': 0,
        'reviews': [
            {
                'author': users[10],
                'content': 'And it looks fabulous! I love, love, love to use naan as a pizza base!',
                },
            {
                'author': users[8],
                'content': 'Each time you post a recipe, I think, “That one’s my favorite!” Then, “that one’s my favorite.” But think this could be my, hands down, favorite. Mmmm. Pizza with a naan crust. Hold me closer, cheesy dancer.',
                },
            {
                'author': users[6],
                'content': 'Yay, I’m so glad you like it, Naan pizzas are so easy and so good. I want one now!',
                },
        ]
      },
    
    {
        'id': 'R0003',
        'name': 'Cajun Crawfish Pasta',
        'image': 'assets/img/one-pot-cajun-pasta.jpg',
        'author': users[11],
        'direction':"""<ul>
        <li>If you don't have cooked noodles on hand already, cook the noodles and cool them before starting to cook the rest of the dish. Set aside for now. You can cook the noodles while prepping the remaining ingredients in the recipe and that should take about the same amount of time.</li>
        <li>In a medium-sized pot, melt the four tablespoons of butter over medium high heat.</li>
        <li>Add the mushrooms, tomatoes, tasso, red bell pepper, green onion, garlic, kosher salt and cayenne pepper. Stir well and cook about 5 minutes, until vegetables are softened.</li>
        <li>Stir in the crawfish tails and continue to cook another 3 minutes until crawfish tails are heated through.</li>
        <li>Add the pasta, parsley (or chives) and parmesan cheese to the pot. Stir well to incorporate all ingredients.</li>
        <li>Serve immediately. Tastes best with garlic bread on the side.</li>
        </ul>""",
        'ingredients': [
            {
                'material': materials[4],
                'unit': 'Cups',
                'quantity': '2'
                },
            {
                'material': materials[5],
                'unit': 'Cup',
                'quantity': '1'
                },
            {
                'material': materials[6],
                'unit': 'Cup',
                'quantity': '1'
                },
            {
                'material': materials[7],
                'unit': 'Cup',
                'quantity': '1/2'
                },
            {
                'material': materials[8],
                'unit': 'Pound or Elbow',
                'quantity': '1'
                },
            {
                'material': materials[9],
                'unit': 'Cup',
                'quantity': '2/3'
                },
            {
                'material': materials[10],
                'unit': 'Cup',
                'quantity': '1'
                },
            ],
        'likes': 0,
        'reviews': [
            {
                'author': users[1],
                'content': 'Have fun tomorrow! Post pictures of it next week. This pasta looks insanely good.',
                },
            {
                'author': users[11],
                'content': 'Thanks so much for visiting and pinning Crawfish is very tasty. Try it if you’re ever able to!',
                },
            {
                'author': users[3],
                'content': 'I made this dish for my mom’s birthday and it was a hit. Absolutely loved it.',
                },
        ]
    },
    
    {
        'id': 'R0004',
        'name': 'Spicy Kimchi Beff Noodles',
        'image': 'assets/img/spicy-kimchi-beef-flavor-instant-noodles.jpg',
        'author': users[5],
        'direction':"""A homemade version of Korean-style spicy beef instant noodles made with short ribs, Korean chili paste, and kimchi.
        <ul>
        <li>Cover kombu with 1 quart cold water. Bring to a boil over medium-high heat. Immediately reduce heat to low and add niboshi or katsuobushi. Let steep for 15 minutes, then drain broth and discard solids. Set aside. While broth simmers, roughly chop 8 scallions and set aside. Finely slice remaining 4 scallions and refrigerate in a sealed container until ready to use.</li>
        <li>Heat 1 tablespoon oil in a heavy Dutch oven or stock pot over high heat until lightly smoking. Add as many short ribs as fit in a single layer and cook, turning occasionally, until browned on all sides, about 10 minutes total. Transfer to a large bowl and repeat until short ribs are all browned.</li>
        <li>Add onion (cut side down) and ginger to pot and cook until well browned, about 5 minutes. Add 6 cloves garlic and roughly chopped scallions and cook, stirring, until lightly browned, about 3 minutes. Return short ribs to pot along with gochujang and doubanjiang. Stir to coat vegetables and beef in spice mixture, then add strained kombu broth and chicken broth, scraping up any browned bits from the bottom of the pot. Bring to a boil, reduce to a bare simmer, cover with the lid slightly cracked, and cook until beef easily separates from the bones, about 4 hours. For best results, allow to cool and transfer to refrigerator overnight.</li>
        <li>When ready to proceed, strain soup through a fine mesh strainer (if refrigerated overnight, you'll have to reheat it slightly until it liquefies). Transfer liquid to a medium pot and skim off any excess fat. Pick out short ribs and transfer to a bowl. Discard remaining solids. When cool enough to handle, pick meat off of short rib bones and transfer to a cutting board. Discard bones. Roughly chop meat and set aside.</li>
            <li>Add kochukaru to broth. Place a fine mesh strainer over the pot and strain kimchi juice into the broth. Finely mince garlic with a garlic press or a microplane and stir into broth. Add soy sauce. Season broth to taste with salt, pepper, and extra kochukaru and gochujang if a spicier flavor is desired. Bring to a simmer and keep warm.</li>
            <li>Heat remaining tablespoon oil in a medium skillet over medium-high heat until shimmering. Add mushrooms and cook until lightly browned, about 4 minutes. Add chopped short rib and cook, tossing occasionally, until crisped in spots and well browned, about 5 minutes longer. Remove from heat and set aside.</li>
            <li>Cook ramen noodles according to package instructions. Divide into individual pre-heated serving bowls. Top with broth, followed by crisped beef and mushrooms, sliced scallions, kimchi, and a soft boiled egg split in half. Serve immediately.</li>
            </ul>"""
            ,
            'ingredients': [
                {
                    'material': materials[23],
                    'unit': 'Cups',
                    'quantity': '2'
                    },
                {
                    'material': materials[24],
                    'unit': 'Cup',
                    'quantity': '1'
                    },
                {
                    'material': materials[25],
                    'unit': 'Cup',
                    'quantity': '1'
                    },
                {
                    'material': materials[26],
                    'unit': 'Cup',
                    'quantity': '1/2'
                    },
                {
                    'material': materials[27],
                    'unit': 'Pound or Elbow',
                    'quantity': '1'
                    },
                {
                    'material': materials[28],
                    'unit': 'Cup',
                    'quantity': '2/3'
                    },
                {
                    'material': materials[29],
                    'unit': 'Cup',
                    'quantity': '1'
                    },
                ],
            'likes': 0,
            'reviews': [
                {
                    'author': users[0],
                    'content': 'Have fun tomorrow! Post pictures of it next week. This pasta looks insanely good.',
                    },
                {
                    'author': users[7],
                    'content': 'Thanks so much for visiting and pinning Crawfish is very tasty. Try it if you’re ever able to!',
                    },
                {
                    'author': users[2],
                    'content': 'I made this dish for my mom’s birthday and it was a hit. Absolutely loved it.',
                    },
            ]
            },    
  
    {
        'id': 'R0005',
        'name': 'Garlic Bread Pizza',
        'image': 'assets/img/garlic_bread_pizza.png',
        'author': users[11],
        'direction':"""This garlic bread pizza recipe is quick, easy and delicious. It's ready in a few minutes and uses just a few simple yet tasty ingredients!
        <ul>
        <li>Pre-heat your oven's broiler.</li>
        <li>Smear the warm red sauce on both halves of the the garlic bread.</li>
        <li>Top with shredded mozzarella.</li>
        <li>Place under the broiler for 3 1/2 to 4 minutes until cheese is melted and browned.</li>
        <li>Top with chopped fresh parsley and serve.</li>
        </ul>"""
        ,
        'ingredients': [
            {
                'material': materials[29],
                'unit': 'Loaf',
                'quantity': '1'
                },
            {
                'material': materials[30],
                'unit': 'Cups',
                'quantity': '2'
                },
            {
                'material': materials[31],
                'unit': 'Cups',
                'quantity': '2'
                },
            {
                'material': materials[32],
                'unit': 'Tablespoons Chopped',
                'quantity': '2'
                },
            ],
        'likes': 0,
        'reviews': [
            {
                'author': users[5],
                'content': 'This recipe took me back to elementary school in the best way possible. We were always so excited for pizza days at lunch, but it was even better when the pizza was a French bread pizza. Nothing compares. I like how you took it to another level by using homemade sauce. I may have to give this a shot the next time I give in to eating pizza!',
                },
            {
                'author': users[11],
                'content': 'Glad you like it, Katherine! let me know if you end up making it!',
                },
            {
                'author': users[9],
                'content': 'I made this for my mom’s birthday and it was a hit. Absolutely loved it.',
                },
        ]
    } ,
    
    {
        'id': 'R0006',
        'name': 'Cane + Nuts Cocktail',
        'image': 'assets/img/cane_nuts_frangelico_cocktail.png',
        'author': users[11],
        'direction':"""Cane + Nuts is a simple, delicious Frangelico cocktail recipe that also includes spiced rum and Coca-Cola. Great holiday drink!
        <ul>
        <li>Add both liquors to the rocks glass filled with ice then top with Coca-Cola.</li>
        <li>Stir gently with a cocktail stirrer, and if desired, garnish with two Luxardo cherries.</li>
        <li>Sip and enjoy!</li>
        </ul>"""
        ,
        'ingredients': [
            {
                'material': materials[29],
                'unit': 'Fluid Ounce',
                'quantity': '1'
                },
            {
                'material': materials[30],
                'unit': 'Fluid Ounce',
                'quantity': '1'
                },
            {
                'material': materials[31],
                'unit': 'Fluid Ounce',
                'quantity': '2'
                },
            {
                'material': materials[32],
                'unit': 'Rocks glass filled with ice',
                'quantity': '1'
                },
            ],
        'likes': 0,
        'reviews': [
            {
                'author': users[5],
                'content': 'This recipe took me back to elementary school in the best way possible. We were always so excited for pizza days at lunch, but it was even better when the pizza was a French bread pizza. Nothing compares. I like how you took it to another level by using homemade sauce. I may have to give this a shot the next time I give in to eating pizza!',
                },
            {
                'author': users[11],
                'content': 'Glad you like it, Katherine! let me know if you end up making it!',
                },
            {
                'author': users[9],
                'content': 'I made this for my mom’s birthday and it was a hit. Absolutely loved it.',
                },
        ]
    },    

    {
        'id': 'R0007',
        'name': 'Japanese Style Grilled Fish',
        'image': 'assets/img/nutrition01.jpg',
        'author': users[10],
        'direction':"""Cane + Nuts is a simple, delicious Frangelico cocktail recipe that also includes spiced rum and Coca-Cola. Great holiday drink!
        <ul>
        <li>Add both liquors to the rocks glass filled with ice then top with Coca-Cola.</li>
        <li>Stir gently with a cocktail stirrer, and if desired, garnish with two Luxardo cherries.</li>
        <li>Sip and enjoy!</li>
        </ul>"""
        ,
        'ingredients': [
            {
                'material': materials[29],
                'unit': 'Fluid Ounce',
                'quantity': '1'
                },
            {
                'material': materials[30],
                'unit': 'Fluid Ounce',
                'quantity': '1'
                },
            {
                'material': materials[31],
                'unit': 'Fluid Ounce',
                'quantity': '2'
                },
            {
                'material': materials[32],
                'unit': 'Rocks glass filled with ice',
                'quantity': '1'
                },
            ],
        'likes': 0,
        'reviews': [
            {
                'author': users[5],
                'content': 'This recipe took me back to elementary school in the best way possible. We were always so excited for pizza days at lunch, but it was even better when the pizza was a French bread pizza. Nothing compares. I like how you took it to another level by using homemade sauce. I may have to give this a shot the next time I give in to eating pizza!',
                },
            {
                'author': users[10],
                'content': 'Glad you like it, Katherine! let me know if you end up making it!',
                },
            {
                'author': users[9],
                'content': 'I made this for my mom’s birthday and it was a hit. Absolutely loved it.',
                },
        ]
    },
  ]


feeds = [
    {   'id': 'A00007',
        'creator': users[5],
        'recipe': recipes[6]
        },
    {   'id': 'A00001',
        'creator': users[1],
        'recipe': recipes[0],
        },
    {   'id': 'A00004',
        'creator': users[10],
        'recipe': recipes[3],
        },
    {   'id': 'A00005',
        'creator': users[6],
        'recipe': recipes[4],
        },
    {    'id': 'A00006',
        'creator': users[10],
        'recipe': recipes[5],
        },
    {   'id': 'A00002',
        'creator': users[2],
        'recipe': recipes[1],
        },
    {   'id': 'A00003',
        'creator': users[5],
        'recipe': recipes[2],
        }, 
]


if __name__ == "__main__":
    recipe = recipes[0]
    print (recipe)