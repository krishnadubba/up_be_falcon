...
class TestUggiPuggiSocialNetwork(testing.TestCase):
    def setUp(self):
        ...
        self.rest_api = 'http://%s:80'%uggipuggi_ip
        print (self.rest_api)
        ...
        for user in dummy_users:
            ...
            for recipe in dummy_recipes:
                ...
                with self.subTest(name=recipe['name']):
                    ...
                    filepath = os.path.join(os.path.dirname(os.path.dirname(here)), 'test_data', 'pasta_copy.jpg')
                    recipe_image2 = open(filepath, 'rb')                    
                    multiple_images = [('recipe_img1', ('pasta.jpg', recipe_image, 'image/jpeg')),
                                       ('recipe_img2', ('pasta_copy.jpg', recipe_image2, 'image/jpeg'))]
                    #multiple_images = {'images': (('pasta.jpg', recipe_image, 'image/jpeg'), ('pasta_copy.jpg', recipe_image2, 'image/jpeg'))}
                    res = requests.post(self.rest_api + '/recipes', 
                                        data=recipe_payload,
                                        files=multiple_images,
                                        headers=header)
                    recipe_image.close()
                    recipe_image2.close()
                    self.assertEqual(200, res.status_code)
